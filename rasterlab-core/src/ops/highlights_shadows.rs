    ...
    fn apply(&self, image: &Image) -> RasterResult<Image> {
        ...
        out.data
            ...
            .for_each(|(dst, src)| {
                ...
                dst[0] = ((r + delta) * 255.0).clamp(0.0, 255.0) as u8;
                dst[1] = ((g + delta) * 255.0).clamp(0.0, 255.0) as u8;
                dst[2] = ((b + delta) * 255.0).clamp(0.0, 255.0) as u8;
                // alpha unchanged
            });
...
mod tests {
    ...
    fn highlights_pulls_bright_down() {
        ...
        assert!(out.data[0] < src.data[0], "bright pixel should darken");
    }
    ...
    fn highlights_leaves_shadows_alone() {
        ...
        let src = grey(30);
        let out = HighlightsShadowsOp::new(-1.0, 0.0).apply(&src).unwrap();
        assert_eq!(out.data[0], src.data[0], "dark pixel should be untouched");
    }
    ...
    fn shadows_lifts_dark() {
        ...
        assert!(out.data[0] > src.data[0], "dark pixel should brighten");
    }
    ...
    fn shadows_leaves_highlights_alone() {
        ...
        let src = grey(230);
        let out = HighlightsShadowsOp::new(0.0, 1.0).apply(&src).unwrap();
        assert_eq!(out.data[0], src.data[0], "bright pixel should be untouched");
    }
