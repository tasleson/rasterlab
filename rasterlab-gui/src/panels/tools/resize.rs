use std::any::Any;

use egui::DragValue;
use rasterlab_core::ops::{ResampleMode, ResizeOp};
use rasterlab_core::traits::operation::Operation;

use super::tool_trait::{Tool, ToolAction, ToolUiCtx};

pub struct ResizeTool {
    pub w: u32,
    pub h: u32,
    pub mode: ResampleMode,
    pub lock_aspect: bool,
}

impl ResizeTool {
    pub fn new() -> Self {
        Self {
            w: 1920,
            h: 1080,
            mode: ResampleMode::Bicubic,
            lock_aspect: true,
        }
    }
}

const MP_PRESETS: &[(&str, u32)] = &[
    ("24 MP", 24_000_000),
    ("20 MP", 20_000_000),
    ("16 MP", 16_000_000),
    ("12 MP", 12_000_000),
    ("10 MP", 10_000_000),
    ("8 MP", 8_000_000),
    ("6 MP", 6_000_000),
    ("5 MP", 5_000_000),
    ("4 MP", 4_000_000),
    ("3 MP", 3_000_000),
    ("2 MP", 2_000_000),
    ("1 MP", 1_000_000),
];

impl Tool for ResizeTool {
    fn id(&self) -> &'static str {
        "resize"
    }
    fn display_name(&self) -> &'static str {
        "⤢  Resize"
    }

    fn render_ui(&mut self, ui: &mut egui::Ui, ctx: &ToolUiCtx<'_>) -> ToolAction {
        let orig_w = self.w;
        let orig_h = self.h;

        if let Some((src_w, src_h)) = ctx.source_dims {
            let src_px = src_w as u64 * src_h as u64;
            let aspect = src_h as f64 / src_w as f64;

            let available: Vec<(&str, u32)> = MP_PRESETS
                .iter()
                .filter(|(_, px)| (*px as u64) < src_px)
                .map(|(lbl, px)| (*lbl, *px))
                .collect();

            if !available.is_empty() {
                let cur_px = orig_w as u64 * orig_h as u64;
                let selected_label = available
                    .iter()
                    .find(|(_, target_px)| {
                        let w = ((*target_px as f64 / aspect).sqrt().round() as u32).max(1);
                        let h = (w as f64 * aspect).round() as u32;
                        (w as u64 * h as u64).abs_diff(cur_px) <= 2
                    })
                    .map(|(lbl, _)| *lbl)
                    .unwrap_or("— Preset —");

                ui.horizontal(|ui| {
                    ui.label("Preset");
                    egui::ComboBox::from_id_salt("resize_mp_preset")
                        .selected_text(selected_label)
                        .show_ui(ui, |ui| {
                            for (lbl, target_px) in &available {
                                let w = ((*target_px as f64 / aspect).sqrt().round() as u32).max(1);
                                let h = (w as f64 * aspect).round() as u32;
                                let hint = format!("{lbl}  ({w}×{h})");
                                if ui.selectable_label(selected_label == *lbl, hint).clicked() {
                                    self.w = w;
                                    self.h = h;
                                }
                            }
                        });
                });
                ui.add_space(4.0);
            }
        }

        let mut w_changed = false;
        let mut h_changed = false;
        egui::Grid::new("resize_grid")
            .num_columns(2)
            .spacing([8.0, 4.0])
            .show(ui, |ui| {
                ui.label("Width");
                let w_resp = ui.add(
                    DragValue::new(&mut self.w)
                        .speed(1)
                        .range(1..=32000_u32)
                        .suffix(" px"),
                );
                w_changed = w_resp.changed();
                ui.end_row();
                ui.label("Height");
                let h_resp = ui.add(
                    DragValue::new(&mut self.h)
                        .speed(1)
                        .range(1..=32000_u32)
                        .suffix(" px"),
                );
                h_changed = h_resp.changed();
                ui.end_row();
                ui.label("Method");
                egui::ComboBox::from_id_salt("resize_mode")
                    .selected_text(match self.mode {
                        ResampleMode::NearestNeighbour => "Nearest",
                        ResampleMode::Bilinear => "Bilinear",
                        ResampleMode::Bicubic => "Bicubic",
                    })
                    .show_ui(ui, |ui| {
                        ui.selectable_value(
                            &mut self.mode,
                            ResampleMode::NearestNeighbour,
                            "Nearest",
                        );
                        ui.selectable_value(&mut self.mode, ResampleMode::Bilinear, "Bilinear");
                        ui.selectable_value(&mut self.mode, ResampleMode::Bicubic, "Bicubic");
                    });
                ui.end_row();
                ui.label("Lock aspect");
                ui.checkbox(&mut self.lock_aspect, "");
                ui.end_row();
            });

        if self.lock_aspect && orig_w > 0 && orig_h > 0 {
            if w_changed && orig_w != self.w {
                let ratio = orig_h as f64 / orig_w as f64;
                self.h = ((self.w as f64 * ratio).round() as u32).max(1);
            } else if h_changed && orig_h != self.h {
                let ratio = orig_w as f64 / orig_h as f64;
                self.w = ((self.h as f64 * ratio).round() as u32).max(1);
            }
        }

        if ui
            .add_enabled(ctx.has_image, egui::Button::new("Apply Resize"))
            .clicked()
        {
            return ToolAction::PushOp(Box::new(ResizeOp::new(self.w, self.h, self.mode)));
        }
        ToolAction::None
    }

    fn load_from_op(&mut self, op: &dyn Operation) -> bool {
        if let Some(o) = op.as_any().and_then(|a| a.downcast_ref::<ResizeOp>()) {
            self.w = o.width;
            self.h = o.height;
            self.mode = o.mode;
            true
        } else {
            false
        }
    }
    fn as_any(&self) -> &dyn Any {
        self
    }
    fn as_any_mut(&mut self) -> &mut dyn Any {
        self
    }
}
