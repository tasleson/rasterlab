...
pub struct CanvasState {
    ...
    last_hash: u64,
    /// Screen-space anchor where the current left-drag started.
    crop_start: Option<Pos2>,
    /// Screen-space tip of the current drag (updated every frame while dragging).
...
impl CanvasState {
    pub fn ui(&mut self, ui: &mut Ui, state: &mut AppState) {
        ...
        // Capture canvas size before any allocations consume the available space.
        ...
        if self.texture.is_none() || new_hash != self.last_hash {
            ...
            self.zoom = fit_zoom(image.width, image.height, canvas_size);
        ...
        if resp.drag_started_by(egui::PointerButton::Primary) {
            // Starting a new drag always replaces any existing selection.
