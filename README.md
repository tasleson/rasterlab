...
- Collections: create one from the sidebar, then right-click a selection in the grid and use **Collections** to put photos in or take them out. A photo can be in as many collections as you like, and each membership is stored in that photo's own `.rlab`, so it survives an index rebuild. Deleting a collection leaves its photos alone.
